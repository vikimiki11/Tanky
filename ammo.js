...
ammoList.push(new Ammo("Small missile", "smallMissile", "smallMissile.png", Infinity, 0, 10,
	() => { return simpleAmmo(null, null, 30, 50) }
));

ammoList.push(new Ammo("Missile", "missile", "missile.png", 0, 1000, 10,
	() => { return simpleAmmo(null, null, 30, 100) }
));

ammoList.push(new Ammo("Volcano Bomb", "volcanoBomb", "volcanoBomb.png", 10, 1000, 10,
	() => {
		return new Promise((resolve) => {
			fireProjectile().then((XYVector) => {
				explosion(XYVector, 30, 50);
				let vector = XYVector[2];
				let speeds = vectorToXY(vector);
				let oneStepBack = [-speeds[0], -speeds[1]];
				let xy = [XYVector[0] + oneStepBack[0], XYVector[1] + oneStepBack[1]];
				Promise.allSettled([
					simpleAmmo(xy, [Math.PI / 4, 2.5], 30, 25),
					simpleAmmo(xy, [-Math.PI / 4, 2.5], 30, 25),
					simpleAmmo(xy, [Math.PI / 2, 2.5], 30, 25),
					simpleAmmo(xy, [-Math.PI / 2, 2.5], 30, 25)
				]).then(resolve);
			}).catch(() => {
...
ammoList.push(new Ammo("Shower", "shower", "shower.png", 10, 1000, 10,
	() => {
		...
		let speedXY = vectorToXY([angle, speed]);
		let speedX = speedXY[0];
		let speedY = speedXY[1];
		return new Promise((resolve) => {
			Promise.allSettled([
				simpleAmmo(null, XYToVector([speedX - 0.3, speedY]), 30, 50),
				simpleAmmo(null, XYToVector([speedX - 0.15, speedY]), 30, 50),
				simpleAmmo(null, XYToVector([speedX, speedY]), 30, 50),
				simpleAmmo(null, XYToVector([speedX + 0.15, speedY]), 30, 50),
				simpleAmmo(null, XYToVector([speedX + 0.3, speedY]), 30, 50)
			]).then(resolve);
		});
...
ammoList.push(new Ammo("Hot Shower", "hotShower", "hotShower.png", 10, 1000, 10,
	() => {
		...
		return new Promise((resolve) => {
			Promise.allSettled([
				simpleAmmo(null, XYToVector([speedX - 0.3, speedY]), 30, 100),
				simpleAmmo(null, XYToVector([speedX - 0.15, speedY]), 30, 100),
				simpleAmmo(null, XYToVector([speedX, speedY]), 30, 100),
				simpleAmmo(null, XYToVector([speedX + 0.15, speedY]), 30, 100),
				simpleAmmo(null, XYToVector([speedX + 0.3, speedY]), 30, 100)
			]).then(resolve);
		});
...
function fireProjectile(xy, speedXY) {
	xy = xy || game.actualPlayer.tank.cannonTip;
	speedXY = speedXY || (() => {
		angle = game.actualPlayer.tank.cannonAngle;
		speed = game.actualPlayer.tank.firePower / 100 * DefaultAmmoSpeed;
		return vectorToXY([angle, speed]);
	})()
	return new Promise((resolve, reject) => {
		projectiles.push(new Projectile(xy, speedXY, resolve, reject));
	});
}
	...
	return new Promise((resolve) => {
		...
		setTimeout(() => {
			...
			for (let y = 1; y <= radius; y++){
				let startx = Math.sqrt(radius**2 - y**2);
				for (let x = Math.round(startx); x >= 0; x--){
...
function simpleAmmo(xy, vector, radius, damage) {			
	return new Promise((resolve) => {
		fireProjectile(xy, vectorToXY(vector)).then((xy) => {
...
const airResistancePerTick = Math.pow(airResistancePerSeccond, 1 / 60);
const Gravity = 0.05;
class Projectile{
	constructor(xy, speedXY, landed, outOfBounds) {
		this.x = xy[0];
		this.y = xy[1];
	...
	tick() {
		this.speedXY[0] *= airResistancePerTick;
		this.speedXY[1] *= airResistancePerTick;
		this.speedXY[1] -= Gravity;
		this.speedXY[0] += game.windCurrent / 3000;
		...
		if (terrain.controlColision(this.x, this.y)[0]) {
			this.landed([this.x, this.y, XYToVector(this.speedXY)]);
			this.DOM.remove();
			this.end = true;
		}
		if(this.y < 0) {

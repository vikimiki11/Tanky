		...
		return new Promise((resolve) => {
			fireProjectile().then((XYVector) => {
				...
				for (let i = 0; i < 4; i++) {
					...
					promises.push(simpleAmmo(xy, vector, 30, 25));
				}
...
ammoList.push(new Ammo("Shower", "shower", "shower.png", 10, 1000, 10,
	() => {
		...
		let promises = [];
		for (let i = -2; i <= 2; i++) {
			promises.push(simpleAmmo(undefined, new Vector(vector.x + 0.15 * i, vector.y), 30, 50));
		}
...
ammoList.push(new Ammo("Hot Shower", "hotShower", "hotShower.png", 10, 1000, 10,
	() => {
		...
		let promises = [];
		for (let i = -2; i <= 2; i++) {
			promises.push(simpleAmmo(undefined, new Vector(vector.x + 0.15 * i, vector.y), 30, 100));
		}
		return Promise.allSettled(promises);
	}
));
...
function fireProjectile(xy, vector) {
	...
	return new Promise((resolve, reject) => {
		projectiles.push(new Projectile(xy, vector, resolve, reject));
	});
}
...
function simpleAmmo(xy, vector, radius, damage) {
	return new Promise((resolve) => {
		fireProjectile(xy, vector).then((xy) => {

...
ammoList.push(new Ammo("Small missile", "smallMissile", "smallMissile.png", Infinity, 0, 10,
	() => {
		return new Promise((resolve) => {
			fireProjectile().then((xy) => {
				explosion(xy, 100, 50).then(resolve);
			}).catch(() => {
				resolve();
...
function fireProjectile(xy, angle, speed) {
	xy = xy || game.actualPlayer.tank.cannonTip;
	angle = angle || game.actualPlayer.tank.cannonAngle;
	speed = speed || game.actualPlayer.tank.firePower / 100 * DefaultAmmoSpeed;
	return new Promise((resolve, reject) => {
		projectiles.push(new Projectile(xy, angle, speed, resolve, reject));
	});
}
...
const Gravity = 0.05;
class Projectile{
	constructor(xy, angle, speed, landed, outOfBounds) {
		this.x = xy[0];
		this.y = xy[1];
	...
	tick() {
		this.speedx *= airResistancePerTick;
		this.speedy *= airResistancePerTick;
		this.speedy -= Gravity;
		this.speedx += game.windCurrent / 3000;
		this.x += this.speedx;
		this.y += this.speedy;
		this.DOM.style.left = this.x + "em";
		this.DOM.style.bottom = this.y + "em";
		if (terrain.controlColision(this.x, this.y)[0]) {
			this.landed([this.x, this.y, this.angle, this.speed]);
			this.DOM.remove();
			this.end = true;
		}
		if(this.y < 0) {
